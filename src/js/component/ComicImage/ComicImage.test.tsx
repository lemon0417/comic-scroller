...
describe('ComicImage type className check', () => {
  it('ComicImage type = undefined => className = ComicImageInit', () => {
    const Cmp = shallow(<ComicImage loading={false} src="" type="" height={0} innerHeight={0} index={0} updateImgType={() => {}} />);
    expect(Cmp.find('.ComicImageInit').length).toBe(1);
  });

  it('ComicImage type = normal => className = ComicImage', () => {
    const Cmp = shallow(<ComicImage loading={false} src="" type={'normal'} height={0} innerHeight={0} index={0} updateImgType={() => {}} />);
    expect(Cmp.find('.ComicImage').length).toBe(1);
  });

  it('ComicImage type = wide => className = ComicImageWide', () => {
    const Cmp = shallow(<ComicImage loading={false} src="" type={'wide'} height={0} innerHeight={0} index={0} updateImgType={() => {}} />);
    expect(Cmp.find('.ComicImageWide').length).toBe(1);
  });

  it('ComicImage type = natural => className = ComicImageNatural', () => {
    const Cmp = shallow(<ComicImage loading={false} src="" type={'natural'} height={0} innerHeight={0} index={0} updateImgType={() => {}} />);
...
describe('ComicImage shows End', () => {
  it('ComicImage type = end => className = ComicImageEnd', () => {
    const Cmp = shallow(<ComicImage loading={false} src="" type={'end'} height={0} innerHeight={0} index={0} updateImgType={() => {}} />);
  ...
  it('show End when props { type = end, loading = true }', () => {
    const Cmp = shallow(<ComicImage loading={true} src="" type={'end'} height={0} innerHeight={0} index={0} updateImgType={() => {}} />);
    expect(Cmp.contains(<div>Loading...</div>)).toBe(false);
    expect(Cmp.find('img').length).toBe(0);
    expect(Cmp.text()).toBe('本 章 結 束');
  });
