...
export function fetchImgListEpic(action$: any, state$: { value: any }) {
  return action$.pipe(
    ...
    mergeMap((action: { index: string | number; }) => {
      ...
      return fetchImgs$(chapter).pipe(mergeMap(({ scriptURL }) => {
        return fetchScript$(scriptURL, chapter).pipe(mergeMap(({ imgList }) => {
...
export function fetchChapterEpic(action$: any) {
  return action$.pipe(
    ...
    mergeMap((action: { chapter: any; }) =>
      fetchImgs$(action.chapter).pipe(mergeMap(({ chapter, scriptURL, comicsID }) => {
        return merge(
          of(updateComicsID(comicsID)),
          fetchScript$(scriptURL, action.chapter).pipe(mergeMap(({ imgList }) => [
            concatImageList(imgList),
            updateRenderIndex(0, 6),
            fetchImgSrc(0, 6),
            startScroll(),
          ])),
          fetchChapterPage$(`${baseURL}/${comicsID}`).pipe(mergeMap(
